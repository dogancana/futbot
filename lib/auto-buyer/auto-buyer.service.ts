import { AutoBuyBidder, AutoBuyBuyNow } from './auto-buyer-jobs';
...
export namespace AutoBuyerService {
  ...
  export function addTarget (target: Target) {
    ...
    const duped: Target = targets.filter(t => t.resourceId === target.resourceId)[0]
    const playerStr = playerService.readable({ id: target.assetId })

    if (!!duped) {
      logger.warn(`${playerStr} is already in auto buy list. Replacing price information with maxPrice=${target.maxPrice}`)
      Object.assign(duped, target)
    } else {
      const playerItemData = StaticItems.itemData[target.assetId]
      if (!playerItemData) {
        throw new Error(`We couldnt find the player with assetId ${target.assetId}`)
      }
      logger.info(`${playerStr} added to list with maxPrice=${target.maxPrice}`)
      targets.push({
        ...target,
        readable: playerStr
      })
