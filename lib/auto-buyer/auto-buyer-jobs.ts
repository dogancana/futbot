...
export class AutoBuyBuyNow extends Job {
  constructor () {
    ...
    const loopOverTargets = async () => {
      const targets = AutoBuyerService.targets
      
      for (const target of targets) {

        const playerStr = playerService.readable({ assetId: target.resourceId })
        const auctions = (await fut.getPlayerTransferData(target.resourceId, 0, { maxb: target.maxPrice }))
        ...
        if (lowest.buyNowPrice <= target.maxPrice) {
          ...
          try {
            ...
            setTimeout(async () => {
              ...
              const justBoughtTarget = purchased.filter(p => p.resourceId === target.resourceId)[0]
